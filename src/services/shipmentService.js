...
const shipmentService = {
    ...
    async exportEmployeeShipments(userId, employeeId, filters = {}) {
        try {
            const { Op } = db.Sequelize;
            ...
            if (filters.sort && filters.sort !== 'none') {
                switch (filters.sort) {
                    case 'orderCountHigh': shipmentData.sort((a, b) => b.orderCount - a.orderCount); break;
                    case 'orderCountLow': shipmentData.sort((a, b) => a.orderCount - b.orderCount); break;
                    case 'WeightHigh': shipmentData.sort((a, b) => b.totalWeight - a.totalWeight); break;
                    case 'WeightLow': shipmentData.sort((a, b) => a.totalWeight - b.totalWeight); break;
                    case 'newest': shipmentData.sort((a, b) => new Date(b.startTime) - new Date(a.startTime)); break;
                    case 'oldest': shipmentData.sort((a, b) => new Date(a.startTime) - new Date(b.startTime)); break;
                }
            }

            return {
                success: true,
                message: 'Lấy toàn bộ shipment của employee thành công',
                exportShipments: shipmentData,
            };

        } catch (error) {
            console.error('exportEmployeeShipments error:', error);
            return { success: false, message: 'Lỗi server' };
        }
    },
}
