...
const EmployeePerformance = () => {
  ...
  const { data = [], total } = useAppSelector(state => state.employee);

  const fetchEmployees = (page = currentPage, search?: string) => {
    const payload: any = {
      page,
      limit: pageSize,
  ...
  const handleViewEmployeeShipmentsDetail = (employeeId: number) => {
    navigate(`/manager/employees/performance/${employeeId}/shipments`);
  };
    ...
    <div style={{ padding: 24, background: "#F9FAFB", borderRadius: 12 }}>
      ...
      <div style={{ marginBottom: 16 }}>
        <SearchFilters
          ...
          filters={{ sort: filterSort, role: filterRole }}
          setFilters={(key, val) => {
