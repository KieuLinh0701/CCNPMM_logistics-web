...
const IncidentDetailModal: React.FC<Props> = ({
  ...
  const statusTag = (status: string) => <Tag color={getStatusColor(status)}>{translateIncidentStatus(status)}</Tag>;
        ...
        <Descriptions bordered column={1} size="middle" labelStyle={{ fontWeight: 'bold', width: 170 }}>
          {incident.order?.trackingNumber && (
            <Descriptions.Item label="Mã đơn hàng">
              <Text style={{ color: '#1890ff', cursor: 'pointer' }} onClick={() => navigate(`/${role}/orders/detail/${incident.order.trackingNumber}`)}>
                {incident.order.trackingNumber}
              </Text>
            </Descriptions.Item>
          )}
          <Descriptions.Item label="Tiêu đề"><Text>{incident.title}</Text></Descriptions.Item>
          <Descriptions.Item label="Loại sự cố">{typeTag(incident.incidentType)}</Descriptions.Item>
          <Descriptions.Item label="Độ ưu tiên">{priorityTag(incident.priority)}</Descriptions.Item>
