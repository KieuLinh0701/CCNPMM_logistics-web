...
import { Form, InputNumber, Select, Button, Typography, Row, Col, message, Card } from "antd";
    ...
    <div style={{ padding: "24px", maxWidth: 900, margin: "0 auto" }}>
      ...
      <Form form={form} layout="vertical">
        ...
        <Form.Item>
          <Button
            type="primary"
            style={{ background: "#1C3D90", color: "white", width: "100%", height: 50, fontSize: 16, marginTop: 20 }}
            onClick={() =>
              form
                .validateFields()
        ...
        {shippingFeeResult !== null && (
          ...
          >
            <Text strong>TỔNG TIỀN CƯỚC VẬN CHUYỂN ƯỚC TÍNH</Text>
            <div style={{ fontSize: 24, color: "#FF4D4F", margin: "8px 0" }}>
              {shippingFeeResult.toLocaleString()} VNĐ
            </div>
            <Text strong>{selectedServiceType?.deliveryTime}</Text>
          </div>
        )}
      </Form>
      {/* Phần lưu ý quan trọng */}
      <Card style={{ marginTop: 32, background: "#fff9e6" }}>
        <Title level={4} style={{ color: "#d48806", marginBottom: 16 }}>
          Lưu ý quan trọng
        </Title>
        <ul style={{ fontSize: 16, lineHeight: 2, color: "#666" }}>
          <li><Text strong>Giá chưa bao gồm VAT (10%)</Text></li>
          <li><Text strong>Phí COD: 2% giá trị hàng hóa</Text></li>
          <li><Text strong>Phí bảo hiểm: 0.5% giá trị hàng hóa</Text></li>
          <li><Text strong>Phí đóng gói: 10,000 VNĐ/kiện</Text></li>
          <li><Text strong>Giá có thể thay đổi theo từng thời điểm</Text></li>
        </ul>
      </Card>
    </div >
  );
